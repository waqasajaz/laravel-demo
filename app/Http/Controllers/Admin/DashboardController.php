...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.dashboard', compact('adminRole', 'actionColumn', 'status'));
    }
    ...
    public function offers_completed($type = 'sale')
    {
        ...
        return view('admin.dashboard', compact('adminRole', 'actionColumn', 'status', 'type'));
    }
    ...
    public function get_offers(Request $request)
    {
	    ...
	    $actionColumn = $request->input("action");

	    $filters = [];
	    if(Auth::guard('admin')->user()->role->type == 'agent'){
		    $agentAssets = property::where('property_deal', $type)
			    ->where('agent_id', Auth::guard('admin')->user()->id)
			    ->pluck('id');
	    ...
	    $filters['step_7_completed'] = 1;
	     ...
	     if($status != "all")
	    {
		    $filters['accept_status'] = ($status=="completed")?0:1;
	    }

	    $count = Offer::where($filters)->where("sold_status", "!=", 1);

	    $filters['property_deal'] = $type;

	    if(isset($filters['property_deal']) && $filters['property_deal'] != ''){
		    $count->whereHas('property', function($q) use($filters) {
	    ...
	    if($total != "")
	    {
		    ...
		    $response =array(
			    "total" => $count,
			    "result" => View::make("admin.list-completed_offers", array(
				    "offers" => $offers,
				    "type"  => $type,
    ...
    public function sold_property($id="")
    {
		if($id != "")
		{
			...
			{
				$offer->sold_status = 1;
				$offer->sold_date = date('Y-m-d H:i:s');
